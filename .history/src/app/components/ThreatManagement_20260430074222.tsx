...
export function ThreatManagement() {
  ...
  const handleSave = () => {
    ...
    if (!editingThreat) {
      ...
      if (threats.length > 0) {
        ...
        nextNumber =
          lastIdParts && lastIdParts.length > 1
    ...
    if (editingThreat) {
      updatedThreats = threats.map((t) =>
        t.id === editingThreat.id ? threatData : t,
      );
    } else {
  ...
  const filteredThreats = threats.filter((t) => {
    ...
    const matchesProbability =
      probabilityFilter === "" || t.probability === probabilityFilter;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Daftar potensi ancaman terhadap aset perusahaan
          </p>
        </div>
        <Button
          onClick={() => setShowAddModal(true)}
          className="bg-[#EB1D29] text-white shadow-md hover:bg-[#c11822] transition-all"
        >
      ...
      <Card className="overflow-x-auto border-none shadow-sm bg-white">
        <Table className="min-w-max">
          <TableHeader className="bg-gray-50/50">
            <TableRow>
              <TableHead className="font-bold text-gray-700 w-[80px]">
                ID
              </TableHead>
            ...
            {filteredThreats.length > 0 ? (
              filteredThreats.map((threat) => (
                ...
                >
                  ...
                  <TableCell
                    className="text-gray-600 text-sm max-w-[250px] truncate"
                  ...
                  <TableCell className="text-center">
                    <Badge
                      variant="outline"
                      className="font-semibold text-gray-500 border-gray-200 bg-gray-50"
                    >
                      {threat.category}
                    </Badge>
                  </TableCell>
          ...
          <Card className="w-full max-w-md p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-gray-900">
                {editingThreat ? "Update Threat" : "Add New Threat"}
              </h3>
              <button
                onClick={closeModal}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Other" className="font-bold text-red-600">
                    + Create New Category
                  </option>
                </select>
              </div>
              ...
              {isCustom && (
                <div className="animate-in fade-in slide-in-from-top-2">
                  <label className="text-xs font-bold text-red-600 block mb-1 uppercase">
                    New Category Name
                  </label>
                  <input
                    type="text"
                ...
                <div className="flex gap-2">
                  {["Low", "Medium", "High", "Not Relevant"].map((p) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, probability: p })
                      }
                      className={`flex-1 py-2 rounded-lg border text-sm font-bold transition-all ${
                        formData.probability === p
                          ? "bg-[#EB1D29] text-white border-[#EB1D29]"
              ...
              <div className="flex gap-3 pt-6 border-t mt-6">
                <Button
                  onClick={handleSave}
                  className="flex-1 bg-[#EB1D29] text-white font-bold py-6 hover:bg-[#c11822]"
                >
